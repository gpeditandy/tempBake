		...
		<view class="p-2 m-2 bg-white rounded">
			...
			<view class="py-2">
				...
				<view class="d-flex a-center mb-3">
					...
					<input maxlength="3" class="category flex-1" type="number" pattern="[0-9]*" @input="handleInput2" placeholder="请输入" placeholder-class="placeholder" v-model="dry_temp" />
					<text>℃</text>
				</view>
				<view class="d-flex a-center">
					...
					<view class="my-cell flex-1 ml-2">
						<picker class="d-flex a-center j-sb " @change="bindPickerChange" :value="timeIndex" :range="timeArr">
							<view class="weui-select">{{dry_duration}}</view>
...
<script>
	export default {
		...
		onLoad(e) {
			...
			this.phone = e.phone
			this.type = e.type
			this.dry_over = '干球超限'+this.dry_temp+'度'
		},
		methods: {
			bindPickerChange(e) {
				this.timeIndex = e.target.value
				this.dry_duration = this.timeArr[this.timeIndex]
				console.log(this.dry_duration)
			},
			handleSave() {
				console.log(this.dry_duration)
				console.log(this.dry_temp)
