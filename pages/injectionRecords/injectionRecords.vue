	...
	<view class="font-md">
			...
			<view class="bg-white m-2 p-2 rounded8">
				<view class="d-flex a-center j-sb border-bottom-mycolor py-2 text-muted"
					...
					<view class="d-flex flex-column">
						...
						<text class="">注水苗数：{{item.sum_amount}}</text>
					</view>
		...
		<template v-else>
			<view class="p-2 my-2 d-flex flex-column a-center bg-white">
				...
				<text class="font-md text-light-muted">暂无注水记录</text>
			</view>
	...
	export default {
		...
		methods: {
			...
			getFloodingLogs() {
				...
				this.$H.get('flooding/flooding/floodingLogs',{
			...
			toDetailPage(item) {
				...
				uni.navigateTo({
					url:'../floodDetail/floodDetail?item=' + strItem + '&timerange=' + this.datetimerange
				})
			}
		}
	}
