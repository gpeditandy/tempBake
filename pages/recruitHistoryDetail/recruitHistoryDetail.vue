<template>
	<view>
			<view class="pb-100">
				...
				<view v-if="itemObj.status==1" :class="itemObj.status==1 ? 'bg-check-yes text-check-yes':''" class="d-flex a-center j-center font-md py-2">审核通过</view>
				<view v-if="itemObj.status==2" :class="itemObj.status==2 ? 'bg-check-no text-check-no':''" class="d-flex a-center j-center font-md py-2">审核不通过</view>
				<view v-if="itemObj.status==-1" :class="itemObj.status==-1 ? 'check-close':''" class="d-flex a-center j-center font-md py-2">已关闭</view>
					...
					<view class="check-bg text-white">
						<view class="d-flex a-center font-lg ml-5 mt-2 mb-1">
							...
							<view class="position-relative">
								<text class="font-md ml-4 bg-white px-1 py text-primary rounded">招人需求</text>
				...
				<view class="font-md bg-white p-2 mx-2 rounded-top position-relative d-flex a-center">
					<image src="../../static/images/village.png" style="width: 50rpx;height: 50rpx;" mode=""></image>
					<view class="ml-2">
						<text class="text-muted mr-5">村屯</text>
						<text>{{itemObj.villageName}}</text>
					</view>
				</view>
				<view class="font-md bg-white p-2 mx-2 rounded-top position-relative d-flex a-center">
					<image src="../../static/images/mi.png" style="width: 50rpx;height: 50rpx;" mode=""></image>
					<view class="ml-2">
						<text class="text-muted mr-5">面积</text>
						<text>{{itemObj.area}}㎡</text>
					</view>
				</view>
				<view class="p-2 m-2 bg-white rounded8">
					...
					<view class="d-flex a-center font-md">
						<text class="px-4 py-1 mr-2 text-primary border-my-primary rounded8 bg-primary-my">{{itemObj.villageName}}</text>
					</view>
				</view>
				<view class="p-2 m-2 bg-white rounded8">
					<view class="d-flex a-center font-md mb-2">
						<image src="../../static/images/find03.png" style="width: 28rpx;height: 28rpx;" mode=""></image>
						<text class="text-muted ml-2">服务类别</text>
					</view>
					<view class="d-flex a-center flex-wrap font-md">
						...
						v-for="(item,index) in itemObj.demandName"
						:key="index"
						>{{item}}</text>
					</view>
				</view>
				<view class="p-2 m-2 bg-white rounded8">
					<view class="d-flex a-center font-md mb-2">
						<image src="../../static/images/marks.png" style="width: 28rpx;height: 28rpx;" mode=""></image>
						<text class="text-muted ml-2">备注</text>
					</view>
					<view class="font-md ml-4 text-muted">
						{{itemObj.remark}}
					</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			handleModify(id) {
				uni.navigateTo({
					url:'../serviceContent/serviceContent?id=' + id
				})
			},
			handleClose() {
				uni.showLoading({
