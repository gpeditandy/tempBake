	...
	<view class="font-md">
			...
			<view class="font-md d-flex a-center">
				...
				<view class="d-flex a-center">
					<picker :range-key="'deviceInfo'" :index="deviceIndex" :range="deviceArr" @change="changeDevice">
						<view class="mx-2 text-muted">{{cutStatus(deviceArr[deviceIndex].deviceInfo)}}</view>
					</picker>
				</view>
			</view>
			<view @click="handleSelected">
				<text class="pl-2 border-left line-h mr-1 text-muted">筛选</text>
		...
		<template v-if="listData.length>0"> <!-- && deviceArr[deviceIndex].status==1 -->
			<view class="bg-white m-2 p-2 rounded8">
				...
				@click="handleWindDetail(item)"
				>
					<view class="d-flex flex-column">
						<text class="mb-1">{{item.startTime}} 开启</text>
						<text>运转时长(h): {{item.duration}}</text>
					</view>
		...
		<template v-else>
			<view class="p-2 my-2 d-flex flex-column a-center bg-white position-relative" style="z-index: 1;">
				...
				<text class="font-md text-light-muted">暂无风机运转记录</text>
			</view>
	...
	export default {
		data() {
			return {
				...
				deviceArr:[{deviceId:'',deviceName:'',deviceInfo:''}],
				deviceIndex:0,
				datetimerange: [],
		...
		methods: {
			...
			handleWindDetail(item) {
				uni.navigateTo({
					url:'../windDetail/windDetail?item=' + JSON.stringify(item) + '&deviceList=' + JSON.stringify(this.deviceList)
				})
			},
			getWindIdList() {
				this.$H.get('seedling/seedlingshed/page',{
					...
					if(res.code != 0) {
						...
						console.log(res)
						this.windflowArr = res.data.list.map(item => item.windflowId)
						this.getWindDeviceInfoList()
					}
				})
			},
			getWindDeviceInfoList() {
				console.log(this.windflowArr.join(','))
				this.$H.get('seedling/seedlingshed/getDeviceList',{
					page:1,
					limit:10,
					deviceIds:this.windflowArr.join(',')
				}).then(res => {
					if(res.code != 0) {
						console.log(res.msg)
					} else {
			...
			getWindLogsList() {
				this.$H.get('seedling/seedlingshed/windFlowLogs',{
